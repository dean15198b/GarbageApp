...
describe("Garbage repository", function () {
  ...
  describe("Get garbages by emptying dates ", function () {
    ...
    it("Get all by until", async function () {
      const garbages = await crud
        .getByEmptyingDates("2021-08-19T12:19:23.000Z", null)
        .map((gar) => {
          delete gar.id;
          return gar;
        });
      expect(JSON.stringify(garbages)).to.equal(
        JSON.stringify(testGarbages.slice(0, 1))
      );
    });
    it("Get all by from", async function () {
      const garbages = await crud
        .getByEmptyingDates(null, "2021-08-21T12:19:23.000Z")
        .map((gar) => {
          delete gar.id;
          return gar;
        });
      expect(JSON.stringify(garbages)).to.equal(
        JSON.stringify(testGarbages.slice(2, 3))
      );
    });
  ...
  describe("Create garbage", function () {
    it("Create", async function () {
      ...
      excpet(JSON.stringify(input)).to.equal(JSON.stringify(garbage));
      expect(
        crud
          .getByEmptyingDates()
          .filter((gar) => JSON.stringify(gar) === JSON.stringify(input))
      ).to.not.be.empty;
    });
  });
  describe("Update garbage", function () {
    it("update exist", async function () {
      ...
      const newEmptyingDate = "2021-08-35T12:19:23.000Z";
      const shouldGarbage = {
        id: garbageId,
      ...
      excpet(JSON.stringify(garbage)).to.equal(JSON.stringify(shouldGarbage));
      excpet(JSON.stringify(foundGarbage)).to.equal(
        JSON.stringify(shouldGarbage)
      );
    });
  ...
  describe("Delete garbage", function () {
    it("Delete exist", async function () {
      ...
      const foundGarbage = await crud.getById(garbageId);
      excpet(JSON.stringify(garbage)).to.equal(JSON.stringify(testGarbages[0]));
      excpet(JSON.stringify(foundGarbage)).to.equal(null);
    });
    it("Delete not exist", async function () {
      const garbage = await crud.deleteById("611d1de714febd230c6d1111");
      const foundGarbage = await crud.getById(garbageId);
      excpet(JSON.stringify(garbage)).to.equal(null);
      excpet(JSON.stringify(foundGarbage)).to.equal(null);
    });
  });
});
